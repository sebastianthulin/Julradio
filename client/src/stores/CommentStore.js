...
CommentStore.fetchReplies = (commentId, limit, cb) => {
  API.get('/comment/replies/' + commentId + '/' + limit, ({comment, replies}) => {
...
CommentStore.post = ({type, target}, text, cb) => {
  API.post('/comment/' + type, {target, text}, cb)
}

CommentStore.reply = (replyTo, text, cb) => {
  API.post('/comment/reply', {replyTo, text}, cb)
}

CommentStore.fetch = ({type, target, limit}, handler) => {
  handler(buildState(target))
  API.get('/comment/' + type, {target, limit}, ({comments, replies, totalComments, totalThreads}) => {
